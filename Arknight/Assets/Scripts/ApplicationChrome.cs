...
class ApplicationChrome
{
	...
	static int SCREENLAYOUT_SIZE_MASK = 0x0f;
	static int SCREENLAYOUT_SIZE_XLARGE = 0x04;
	//public static int checkNavigtionBar()
	//{
	...
	//	var view = window.Call<AndroidJavaObject>("getDecorView");
	//	var option = view.Call<int>("getSystemUiVisibility");
	//	bool isImmersiveModeEnabled = ((option | SYSTEM_UI_FLAG_IMMERSIVE_STICKY) == option);
	//	return option;
	//}
	static CanvasScaler _canvasScreen;
	static RectTransform _rect;
	static bool _statusBarR;
	static bool _navigationBarR;
	static bool _start = false;
	public static int getStatusbarHeight()
	{
		var androidJavaClass = new AndroidJavaClass("com.unity3d.player.UnityPlayer");
		var unityActivity = androidJavaClass.GetStatic<AndroidJavaObject>("currentActivity");

		var statusHeight = 0;
		var screenSizeType = (unityActivity.Call<AndroidJavaObject>("getResources").Call<AndroidJavaObject>("getConfiguration").Get<int>("screenLayout") & SCREENLAYOUT_SIZE_MASK);
		if (screenSizeType != SCREENLAYOUT_SIZE_XLARGE)
		{
			var resourceId = unityActivity.Call<AndroidJavaObject>("getResources").Call<int>("getIdentifier", "status_bar_height", "dimen", "android");
			if (resourceId > 0)
			{
				statusHeight = unityActivity.Call<AndroidJavaObject>("getResources").Call<int>("getDimensionPixelSize", resourceId);
			}
		}
		return statusHeight;
	}
	public static int getNavigationBarHeight()
	{
		...
		if (screenSizeType != SCREENLAYOUT_SIZE_XLARGE)
		{
			var resourceId = unityActivity.Call<AndroidJavaObject>("getResources").Call<int>("getIdentifier", "navigation_bar_height", "dimen", "android");
			if (resourceId > 0)
			{
				bottomBarHeight = unityActivity.Call<AndroidJavaObject>("getResources").Call<int>("getDimensionPixelSize", resourceId);
			}
		}
		return bottomBarHeight;
	}
	public static void setSystemUiVisibility(CanvasScaler canvas, RectTransform rect, bool statusBar, bool navigationBar)
	{
		_canvasScreen = canvas;
		_rect = rect;
		_statusBarR = statusBar;
		_navigationBarR = navigationBar;
		...
		screenSize = new Vector2(1080, (int)(1080 * h));
		float hh = (1080 * h) / (float)Screen.height;
		status = (int)(status * hh);
		navigation = (int)(navigation * hh);

		canvas.referenceResolution = screenSize;

		if (statusBar && navigationBar)
		{
			Screen.fullScreen = false;

			rect.sizeDelta = new Vector2(screenSize.x, screenSize.y - (status + navigation));
			rect.anchoredPosition = new Vector2(0, (navigation - status) / 2);

			statusBarState = States.TranslucentOverContent;
			navigationBarState = States.TranslucentOverContent;
		}
		else if (!statusBar && !navigationBar)
		{
			...
			Screen.fullScreen = true;
			rect.sizeDelta = new Vector2(screenSize.x, screenSize.y);
			rect.anchoredPosition = new Vector2(0, 0);
			statusBarState = States.Hidden;
			navigationBarState = States.Hidden;
			Screen.fullScreen = true;
		}
		else if (statusBar && !navigationBar)
		{
			...
			rect.sizeDelta = new Vector2(screenSize.x, screenSize.y - (status));
			rect.anchoredPosition = new Vector2(0, -status * 0.5f);
			statusBarState = States.TranslucentOverContent;
			Screen.fullScreen = true;
			navigationBarState = States.Hidden;
		}
		...
		{
			Screen.fullScreen = false;
			rect.sizeDelta = new Vector2(screenSize.x, screenSize.y - (navigation));
			rect.anchoredPosition = new Vector2(0, +(navigation / 2));
			statusBarState = States.Hidden;
			navigationBarState = States.TranslucentOverContent;
		}
		_start = true;
	}
	public static void setSystemUiVisibilityChange()
	{
		setSystemUiVisibility(_canvasScreen, _rect, _statusBarR, _navigationBarR);
	}
}